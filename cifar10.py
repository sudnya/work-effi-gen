...
def _variable_on_cpu(name, shape, initializer):
    ...
    with tf.device('/cpu:0'):
        dtype = tf.float16 if config.get("use_fp16", False) else tf.float32
...
def process_inputs(is_training):
    ...
    """
    if not config.get("data_dir", "/tmp"):
        raise ValueError('Please supply a data_dir')
    data_dir = os.path.join(config.get("data_dir", "/tmp"), 'cifar-10-batches-bin')
    images, labels = cifar10_input.process_inputs(data_dir=data_dir,
...
def inference(images):
    ...
    with tf.variable_scope('local3') as scope:
        ...
        reshape = tf.reshape(pool2, [config.get("batch_size",32), -1])
